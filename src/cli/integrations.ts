...
function asideProfileQuery(profile: string | undefined, client: string | undefined): string {
...
export async function handleClientIntegrationCommand(
  ...
  return runCliAction(async () => {
    ...
    if (action === "status" || action === "show" || action === "list") {
      ...
      const profileQuery = asideProfileQuery(profile, client);
      rejectArgs(args, CLIENT_USAGE);
      const path = client
        ? `/api/client-integrations/${encodeURIComponent(client)}${profileQuery ? `?${profileQuery}` : ""}`
    ...
    if (action === "history" || action === "journal") {
      ...
      const profileQuery = asideProfileQuery(profile, client);
      rejectArgs(args, CLIENT_USAGE);
      const query = client ? `?client=${encodeURIComponent(client)}${profileQuery ? `&${profileQuery}` : ""}` : "";
    ...
    if (action === "restore") {
      ...
      const client = takeOption(args, "--client");
      const profileQuery = asideProfileQuery(profile, client);
      if (client !== undefined && !profileQuery) throw new CliUsageError("restore --client requires --profile", CLIENT_USAGE);
      rejectArgs(args, CLIENT_USAGE);
      if (!opId) throw new CliUsageError("--op <opId> is required", CLIENT_USAGE);
      const result = await runtimeRequest(`/api/client-integrations/restore${profileQuery ? `?client=aside&${profileQuery}` : ""}`, {
        method: "POST",
    ...
    const result = await runtimeRequest(`/api/client-integrations/${encodeURIComponent(client)}${profileQuery ? `?${profileQuery}` : ""}`, {
      method: "PUT",
