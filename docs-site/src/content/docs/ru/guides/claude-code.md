...
Это изменение псевдонимов не исправляет отдельный запрос [#3646](https://github.com/lidge-jun/opencodex/issues/3646) о повторной передаче
...
Неразрешённые управляемые псевдонимы Desktop с датой/хешем отклоняются с HTTP 400 до удаления даты или fallback, как для Messages, так и для count-tokens. Точные записи оператора в `modelMap` и распознанные реальные ID Anthropic обрабатываются как обычно.
